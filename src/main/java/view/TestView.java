...
import org.eclipse.jdt.internal.ui.javaeditor.JavaMoveLinesAction;
import org.eclipse.jdt.internal.ui.javaeditor.JavaSourceViewer;
import org.eclipse.jdt.internal.ui.javaeditor.JavaTemplatesPage;
...
//		boolean closeBrackets = preferenceStore
//				.getBoolean(PreferenceConstants.EDITOR_CLOSE_BRACKETS);
//		boolean closeStrings = preferenceStore
//				.getBoolean(PreferenceConstants.EDITOR_CLOSE_STRINGS);
//		boolean closeAngularBrackets = JavaCore.VERSION_1_5
//				.compareTo(preferenceStore.getString(JavaCore.COMPILER_SOURCE)) <= 0;
//
//		
...
//
//		fContainerManager = new ContainerManager(viewer.getDocument(),
...
public class TestView extends CompilationUnitEditor {
	...
	class TestViewAdaptedSourceViewer extends JavaSourceViewer {
		public TestViewAdaptedSourceViewer(Composite parent,
		...
		public void verifyKey(VerifyEvent event) {
			...
			try {
				...
				int nextToken = scanner.nextToken(offset + length, endLine
						.getOffset()
						+ endLine.getLength());
				String next = nextToken == Symbols.TokenEOF ? null : document
						.get(offset, scanner.getPosition() - offset).trim();
				int prevToken = scanner.previousToken(offset - 1, startLine
						.getOffset());
				int prevTokenOffset = scanner.getPosition() + 1;
				String previous = prevToken == Symbols.TokenEOF ? null
						: document.get(prevTokenOffset,
	...
	private class RememberedSelection {
		...
		public void remember() {
			 ...
			 * may be called inside an asynchronous call posted to the UI
			 * thread, so protect against intermediate disposal of the editor.
			 */
	...
	private class RememberedOffset {
		...
		/**
		 * Computes the element line of a java element (the start of the
		 * element, or the line with the element's name range).
		 * 
		 * @param document
		 ...
		 *             if getting the name range of the element fails
		 * @since 3.2
		 */
		private int getElementLine(IDocument document, IJavaElement element)
	...
	protected void createActions() {
		...
		if (action != null) {
			...
			action.setText(TestViewMessages.getBundleForConstructedKeys()
					.getString("ContentAssistContextInformation.label")); //$NON-NLS-1$
			action.setToolTipText(TestViewMessages
					.getBundleForConstructedKeys().getString(
							"ContentAssistContextInformation.tooltip")); //$NON-NLS-1$
			action.setDescription(TestViewMessages
					.getBundleForConstructedKeys().getString(
							"ContentAssistContextInformation.description")); //$NON-NLS-1$
		}

		action = new TextOperationAction(TestViewMessages
		...
		PlatformUI.getWorkbench().getHelpSystem().setHelp(action,
				IJavaHelpContextIds.UNCOMMENT_ACTION);

		action = new ToggleCommentAction(TestViewMessages
		...
		action = new AddBlockCommentAction(TestViewMessages
				.getBundleForConstructedKeys(), "AddBlockComment.", this); //$NON-NLS-1$
		action
				.setActionDefinitionId(IJavaEditorActionDefinitionIds.ADD_BLOCK_COMMENT);
		...
		action = new RemoveBlockCommentAction(TestViewMessages
				.getBundleForConstructedKeys(), "RemoveBlockComment.", this); //$NON-NLS-1$
		action
				.setActionDefinitionId(IJavaEditorActionDefinitionIds.REMOVE_BLOCK_COMMENT);
		...
		action = new IndentAction(TestViewMessages
		...
		JavaMoveLinesAction[] moveLinesActions = JavaMoveLinesAction
				.createMoveCopyActionSet(TestViewMessages
						.getBundleForConstructedKeys(), this);
		ResourceAction rAction = moveLinesActions[0];
		rAction
				.setHelpContextId(IAbstractTextEditorHelpContextIds.MOVE_LINES_ACTION);
		rAction
				.setActionDefinitionId(ITextEditorActionDefinitionIds.MOVE_LINES_UP);
		...
		rAction
				.setHelpContextId(IAbstractTextEditorHelpContextIds.MOVE_LINES_ACTION);
		rAction
				.setActionDefinitionId(ITextEditorActionDefinitionIds.MOVE_LINES_DOWN);
		setAction(ITextEditorActionConstants.MOVE_LINE_DOWN, rAction);

		rAction = moveLinesActions[2];
		rAction
				.setHelpContextId(IAbstractTextEditorHelpContextIds.COPY_LINES_ACTION);
		rAction
				.setActionDefinitionId(ITextEditorActionDefinitionIds.COPY_LINES_UP);
		...
		rAction
				.setHelpContextId(IAbstractTextEditorHelpContextIds.COPY_LINES_ACTION);
		rAction
				.setActionDefinitionId(ITextEditorActionDefinitionIds.COPY_LINES_DOWN);
		...
		fCorrectionCommands = new CorrectionCommandInstaller(); // allow
		// shortcuts for
		// quick
		// fix/assist
		...
		if (p.isDeleted(getEditorInput())) {
			...
			if (isSaveAsAllowed()) {
				...
				MessageDialog
						.openError(
								shell,
								TestViewMessages.CompilationUnitEditor_error_saving_title1,
								TestViewMessages.CompilationUnitEditor_error_saving_message1);
			}
	...
	/*
	 * @see
	 * org.eclipse.ui.texteditor.AbstractTextEditor#openSaveErrorDialog(java
	 * .lang.String, java.lang.String, org.eclipse.core.runtime.CoreException)
	 * 
	 * @since 3.3
	 */
	protected void openSaveErrorDialog(String title, String message,
		...
		if (JavaUI.ID_PLUGIN.equals(status.getPlugin())
				&& (status.getCode() == IJavaStatusConstants.EDITOR_POST_SAVE_NOTIFICATION || status
						.getCode() == IJavaStatusConstants.EDITOR_CHANGED_REGION_CALCULATION)) {
	...
	/**
	 * Opens a warning dialog informing about a failure during handling of save
	 * listeners.
	 * 
	 * @param title
	 *            the dialog title
	 ...
	 * @since 3.4
	 */
	private void openSaveListenerWarningDialog(String title, String message,
			CoreException exception) {
		final String linkText;
		final IJavaProject javaProject = getInputJavaElement().getJavaProject();
		IProject project = javaProject != null ? javaProject.getProject()
				: null;
		final boolean hasProjectSettings = project != null
				&& CleanUpPreferenceUtil.hasSettingsInScope(new ProjectScope(
						project));
		if (exception.getStatus().getCode() == IJavaStatusConstants.EDITOR_POST_SAVE_NOTIFICATION) {
			message = TestViewMessages.CompilationUnitEditor_error_saving_participant_message;
			if (hasProjectSettings)
				linkText = TestViewMessages.CompilationUnitEditor_error_saving_participant_property_link;
			else
				linkText = TestViewMessages.CompilationUnitEditor_error_saving_participant_link;
		} else {
			message = TestViewMessages.CompilationUnitEditor_error_saving_editedLines_calculation_message;
			if (hasProjectSettings)
				linkText = TestViewMessages.CompilationUnitEditor_error_saving_editedLines_calculation_property_link;
			else
				linkText = TestViewMessages.CompilationUnitEditor_error_saving_editedLines_calculation_link;
		}

		IStatus status = exception.getStatus();
		int mask = IStatus.WARNING | IStatus.ERROR;
		ErrorDialog dialog = new ErrorDialog(getSite().getShell(), title,
				message, status, mask) {
			protected Control createMessageArea(Composite parent) {
				Control result = super.createMessageArea(parent);

				// Panic code: use 'parent' instead of 'result' in case super
				// implementation changes in the future
				new Label(parent, SWT.NONE); // filler as parent has 2 columns
				// (icon and label)
						...
						if (hasProjectSettings)
							PreferencesUtil
									.createPropertyDialogOn(
											getShell(),
											javaProject,
											SaveParticipantPreferencePage.PROPERTY_PAGE_ID,
											null, null).open();
						else
							PreferencesUtil
	...
	protected void doSetInput(IEditorInput input) throws CoreException {
		...
		if (fJavaEditorErrorTickUpdater != null)
			fJavaEditorErrorTickUpdater
					.updateEditorImage(getInputJavaElement());
	}

	/*
	 * @see
	 * org.eclipse.jdt.internal.ui.javaeditor.JavaEditor#installOverrideIndicator
	 * (boolean)
	 * 
	 * @since 3.0
	 */
	...
	/*
	 * @see
	 * org.eclipse.ui.texteditor.AbstractTextEditor#installTabsToSpacesConverter
	 * ()
	 * 
	 * @since 3.3
	 */
	protected void installTabsToSpacesConverter() {
		...
		if (config != null && sourceViewer instanceof ITextViewerExtension7) {
			...
			if (provider instanceof ICompilationUnitDocumentProvider) {
				...
				tabToSpacesConverter.setLineTracker(cup
	...
	protected boolean isTabsToSpacesConversionEnabled() {
		...
		IJavaProject project = element == null ? null : element
				.getJavaProject();
		String option;
		if (project == null)
	...
	protected void handlePreferenceStoreChanged(PropertyChangeEvent event) {
		...
		try {
			...
			TestViewAdaptedSourceViewer asv = (TestViewAdaptedSourceViewer) getSourceViewer();
			if (asv != null) {
				...
				if (CLOSE_BRACKETS.equals(p)) {
					fBracketInserter
							.setCloseBracketsEnabled(getPreferenceStore()
									.getBoolean(p));
					return;
				}
				...
				if (CLOSE_STRINGS.equals(p)) {
					fBracketInserter
							.setCloseStringsEnabled(getPreferenceStore()
									.getBoolean(p));
					return;
				}
				...
				if (JavaCore.COMPILER_SOURCE.equals(p)) {
					...
					fBracketInserter
							.setCloseAngularBracketsEnabled(closeAngularBrackets);
				}
				...
				if (PreferenceConstants.EDITOR_SMART_TAB.equals(p)) {
					if (getPreferenceStore().getBoolean(
						...
						setActionActivationCode(
								"IndentOnTab", '\t', -1, SWT.NONE); //$NON-NLS-1$
					} else {
	...
	/*
	 * @see
	 * org.eclipse.jdt.internal.ui.javaeditor.JavaEditor#createJavaSourceViewer
	...
	protected JavaSourceViewer createJavaSourceViewer(Composite parent,
		...
		return new TestViewAdaptedSourceViewer(parent, verticalRuler,
				overviewRuler, isOverviewRulerVisible, styles, store);
	}
	...
	/*
	 * @see
	 * org.eclipse.jdt.internal.ui.javaeditor.JavaEditor#getAdapter(java.lang
	 * .Class)
	 */
	...
	/*
	 * @see
	 * org.eclipse.jdt.internal.ui.javaeditor.JavaEditor#createNavigationActions
	 * ()
	 */
	 ...
	 * 
	 * @return the correction command installer, or <code>null</code> if there
	 *         is none
	 * @since 3.5
	 */
